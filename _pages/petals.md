---
layout: page
title: petals
description: "Monet once said, 'I must have flowers, always and always.' Here are some bouquets I made!"
permalink: /petals/
nav: true
nav_order: 5
---
<div style="columns: 3; gap: 12px;">
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/hibi.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">beach sunset hibiscus | 7.28.26</p>
  </div>
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/thebou.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">arya + ananya's summer in new york | 6.24.26</p>
  </div>
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/bbpeony.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">peonies + baby's breath | 6.21.26 </p>
  </div>
</div>

<div style="columns: 3; gap: 12px;">
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/flower1.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">lilies, hydrangeas, roses | 4.26.26</p>
  </div>
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/flower2.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">amelia and i's joint bouquet | 11.25.25</p>
  </div>
  <div style="break-inside: avoid; margin-bottom: 12px;">
    {% include figure.liquid path="assets/img/flower3.jpg" class="img-fluid rounded" %}
    <p style="font-size: 0.8rem; text-align: center; margin-top: 4px;">rachel and i + azeeza pop-up | summer '25</p>
  </div>
</div>
